/*!
 * Copyright 2016, TK ipking
 * https://git.oschina.net/ipking/pjax
 */
...
;(function($, window, document,undefined) {
	var Pjax = function(el, opt) {
		...
		options = $.extend({}, self.defaults, opt);
		self.run=function() {
			if(options.event===0){
				self.bind();
			}else{
				self.url(options.event);
			}
		...
		self.click = function(e) {
			if (!window.History.enabled) {
				options.error('history disabled');
				return true;
			}
			
			options.before();
			self.getData(this.href);
		...
		self.getData = function(url) {
			var data = self.getStorage(url);
			if(data){
				var g = JSON.parse(data);
				if(g.data){
					response = JSON.parse(g.data);
					if(!response.title)response.title = $('title').html();
					window.History.pushState({href:url}, response.title, url);
		...
		self.ajax = function(url) {
			$.ajax({
				...
				success:function(response) {
					if(!response.title)response.title = $('title').html();
					window.History.pushState({href:url}, response.title, url);
					self.setStorage(url,response);
					options.success(response);
				},
				error:function(xhr,info,e) {
					options.error(xhr,info,e);
				},
		...
		self.href = function(url){
			...
			if(data){
				...
				$.ajax({
					...
					success:function(e) {
						g.data = JSON.stringify(e);
						var s = JSON.stringify(g);
						window.sessionStorage[url] = s;
						
						var s = g.success;
						s = 'var f = ' + s;
						eval('{'+  s  +'}'); 
						if(!e.title)e.title = $('title').html();
						window.History.pushState({href:url}, e.title, url);
						f(e);
					}
				});
			}else{
		...
		self.reLoad = function(url){
			...
			if(data){
				...
				$.ajax({
					...
					success:function(response) {
						g.data = JSON.stringify(response);
						var s = JSON.stringify(g);
						window.sessionStorage[url] = s;
					}
				});
		...
		self.setStorage = function(key,value){
			if(window.sessionStorage && options.openStorage){
			}else{
				return false;
			}
			var lens = 1024 * 1024 * 5 - unescape(encodeURIComponent(JSON.stringify(sessionStorage))).length;
			if(lens < 0){
				options.openStorage = 0;
				return false;
			}
			var data = JSON.stringify(value);
			if(lens - data.length*15 < 0){
				options.openStorage = 0;
				return false;
			}
			var S = {
			...
			var g = JSON.stringify(S);
			window.sessionStorage[key] = g;
	...
	$.fn.pjax = function(options) {
		...
		var p = new Pjax(this, options);
		
		return p.run();
	}
	
	window.onpopstate = function(event) {
		var d = window.sessionStorage[document.location];
		if(!d) {location.href = document.location;return false;}
		var g = JSON.parse(d);
		var s = g.success;
		s = 'var f = ' + s;
		eval('{'+  s  +'}'); 
